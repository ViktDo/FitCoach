...
import { API_BASE, cleanToken, getRole, hasPdn, normalizePhone, apiGet, apiPost, stripEq, goHome } from '@/app/lib/client';
...
export default function ProfilePage() {
  ...
  const [cInst, setCInst] = useState('');          // url
  const [cTgCh, setCTgCh] = useState('');          // @handle | url
  const [cTgLink, setCTgLink] = useState('');      // url

  const token = useMemo(() => cleanToken(localStorage.getItem('session_token')), []);

  useEffect(() => {
    if (!token) { window.location.replace('/onboard/auth'); return; }
    const r = getRole();
    if (r === 'pending') { window.location.replace('/onboard/role'); return; }
    if (!hasPdn()) { window.location.replace('/onboard/consent'); return; }
    setRole(r);
  ...
  // ── нормализация соцсетей (UX-подсказки) ──────────────────────────────
  ...
  function toTgChannelVal(v: string) {
    const s = (v||'').trim();
    if (!s) return '';
    if (/^https?:\/\//i.test(s)) return s; // разрешаем полную ссылку
    // если ввели @handle → оставляем как @handle (бэку вернём как строку)
    // если ввели "my_channel" → тоже ок
    return s;
  }
    ...
    <main className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-2xl p-6">
        <div className="flex items-start justify-between mb-2">
          ...
          <button onClick={goHome} className="ml-4 shrink-0 px-3 py-2 rounded-xl border text-sm hover:bg-gray-50">← Назад</button>
        </div>
        ...
        <div className="space-y-4 mb-6">
          ...
          <p className="text-xs text-gray-500">Данные используются только для работы сервиса (уведомления, связь с тренером).</p>
        </div>

        {/* ATHLETE */}
        {role === 'athlete' && (
          <form onSubmit={e=>{e.preventDefault(); void saveAthlete();}} className="space-y-4">
            <div className="text-lg font-semibold mb-2">Спортсмен</div>
            <div className="grid grid-cols-2 gap-4">
              <label className="block">
                <span className="block mb-1 font-medium">Рост (см)</span>
                <input value={aHeight} onChange={e=>setAHeight(e.target.value)} type="number" min={100} max={250} className="w-full border rounded-xl px-3 py-2" />
              </label>
              <label className="block">
                <span className="block mb-1 font-medium">Вес (кг)</span>
                <input value={aWeight} onChange={e=>setAWeight(e.target.value)} type="number" step="0.1" min={30} max={300} className="w-full border rounded-xl px-3 py-2" />
              </label>
            </div>
            <label className="block">
              <span className="block mb-1 font-medium">Цель</span>
              <input value={aGoal} onChange={e=>setAGoal(e.target.value)} className="w-full border rounded-xl px-3 py-2" placeholder="Напр. «Набор мышечной массы»" />
            </label>
            <label className="block">
              <span className="block mb-1 font-medium">Заметки</span>
              <textarea value={aNotes} onChange={e=>setANotes(e.target.value)} rows={3} className="w-full border rounded-xl px-3 py-2" />
            </label>
            <div className="flex gap-3">
              <button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-2 font-semibold">Сохранить профиль спортсмена</button>
              <button type="button" onClick={goHome} className="px-4 py-2 rounded-xl border hover:bg-gray-50">Назад</button>
            </div>
          </form>
        )}
        ...
        {/* COACH */}
        {role === 'coach' && (
          <form onSubmit={e=>{e.preventDefault(); void saveCoach();}} className="space-y-4">
            <div className="text-lg font-semibold mb-2">Тренер</div>
            <label className="block">
              <span className="block mb-1 font-medium">О себе</span>
              <textarea value={cBio} onChange={e=>setCBio(e.target.value)} rows={3} className="w-full border rounded-xl px-3 py-2" placeholder="Краткое био, специализация" />
            </label>
            <label className="block">
              <span className="block mb-1 font-medium">Опыт, лет</span>
              <input value={cExp} onChange={e=>setCExp(e.target.value)} type="number" min={0} max={60} className="w-full border rounded-xl px-3 py-2" />
            </label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="block">
                ...
                <input
                  ...
                  onBlur={e=>setCInst(prev => prev ? ensureHttps(toInstagramUrl(prev)) : '')}
                  className="w-full border rounded-xl px-3 py-2" placeholder="ваш @ник или ссылка" />
                <p className="text-xs text-gray-500 mt-1">Можно указать @ник — ссылка сформируется автоматически.</p>
              </label>
              <label className="block">
                ...
                <input
                  ...
                  onBlur={e=>setCTgCh(prev => prev?.startsWith('http') ? ensureHttps(prev) : prev)}
                  className="w-full border rounded-xl px-3 py-2" placeholder="@my_channel или полная ссылка" />
                <p className="text-xs text-gray-500 mt-1">Можно @handle или полную ссылку (https://t.me/…)</p>
              </label>
            </div>
            ...
            <label className="block">
              ...
              <input
                ...
                onBlur={e=>setCTgLink(prev => prev ? ensureHttps(toTgUserUrl(prev)) : '')}
                className="w-full border rounded-xl px-3 py-2" placeholder="@username или https://t.me/username" />
              <p className="text-xs text-gray-500 mt-1">Можно указать @ник — ссылка t.me сформируется сама.</p>
            </label>

            <div className="flex gap-3">
              <button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-2 font-semibold">Сохранить профиль тренера</button>
              <button type="button" onClick={goHome} className="px-4 py-2 rounded-xl border hover:bg-gray-50">Назад</button>
            </div>
          </form>
        )}
